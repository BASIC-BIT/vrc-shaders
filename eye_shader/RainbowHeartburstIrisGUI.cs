...
public class RainbowHeartburstIrisGUI : ShaderGUI
{
    ...
    private MaterialProperty _EnableTopShading;
    private MaterialProperty _TopShadingIntensity;
    private MaterialProperty _TopShadingHeight;
    private MaterialProperty _TopShadingSoftness;
    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
    {
        ...
        DrawMirrorSection(materialEditor, material, properties); // Corrected order
        ...
        EditorGUILayout.Space(); // Added space

        // Draw Top Shading section (Moved to after Environment)
        DrawTopShadingSection(materialEditor, material, properties);
    }
    ...
    private void FindProperties(MaterialProperty[] properties)
    {
        ...
        // Find Top Shading properties
        _EnableTopShading = FindProperty("_EnableTopShading", properties);
        _TopShadingIntensity = FindProperty("_TopShadingIntensity", properties);
        _TopShadingHeight = FindProperty("_TopShadingHeight", properties);
        _TopShadingSoftness = FindProperty("_TopShadingSoftness", properties);
    }
    ...
    // --- NEW SECTION ---
    private void DrawTopShadingSection(MaterialEditor materialEditor, Material material, MaterialProperty[] properties)
    {
        // Use the toggle as the section header
        bool toggleValue = _EnableTopShading.floatValue > 0.5f;
        EditorGUILayout.BeginHorizontal();
        bool newToggleValue = EditorGUILayout.ToggleLeft("Top Shading", toggleValue, EditorStyles.boldLabel);
        EditorGUILayout.EndHorizontal();

        // Update shader keyword if toggle changed
        if (newToggleValue != toggleValue)
        {
            _EnableTopShading.floatValue = newToggleValue ? 1.0f : 0.0f;
            SetKeyword(material, "_ENABLE_TOP_SHADING", newToggleValue);
        }
        ...
        {
            EditorGUI.indentLevel++;
            materialEditor.ShaderProperty(_TopShadingIntensity, "Intensity");
            materialEditor.ShaderProperty(_TopShadingHeight, "Height");
            materialEditor.ShaderProperty(_TopShadingSoftness, "Softness");
            EditorGUI.indentLevel--;
        }
    }
    // --- END NEW SECTION ---
...
} // End of class RainbowHeartburstIrisGUI